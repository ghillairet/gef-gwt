...
/**
 * <code>FillLayout</code> is the simplest layout class. It lays out controls in
 * a single row or column, forcing them to be the same size.
 * <p>
 ...
 * <code>FillLayout</code> can also be used when a <code>Composite</code> only
 * has one child. For example, if a <code>Shell</code> has a single
 * <code>Group</code> child, <code>FillLayout</code> will cause the
 * <code>Group</code> to completely fill the <code>Shell</code> (if margins are
 * 0).
 * </p>
 * <p>
 * Example code: first a <code>FillLayout</code> is created and its type field
 * is set, and then the layout is set into the <code>Composite</code>. Note that
 * in a <code>FillLayout</code>, children are always the same size, and they
...
public final class FillLayout extends Layout {
	...
	Point computeChildSize(Control control, int wHint, int hHint,
			boolean flushCache) {
	...
	public String toString() {
		...
		string += "type="
				+ ((type == SWT.VERTICAL) ? "SWT.VERTICAL" : "SWT.HORIZONTAL")
				+ " ";
		if (marginWidth != 0)

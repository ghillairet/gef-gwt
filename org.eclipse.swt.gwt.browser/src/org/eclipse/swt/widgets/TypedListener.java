	...
	public void handleEvent(Event e) {
		...
		case SWT.Collapse: {
			if (eventListener instanceof TreeListener) {
				...
				((ExpandListener) eventListener).itemCollapsed(new ExpandEvent(
						e));
			}
			break;
		}
		...
		case SWT.Dispose: {
			((DisposeListener) eventListener)
					.widgetDisposed(new DisposeEvent(e));
			break;
		}
		case SWT.DragDetect: {
			((DragDetectListener) eventListener)
		...
		case SWT.Expand: {
			if (eventListener instanceof TreeListener) {
				...
				((ExpandListener) eventListener)
						.itemExpanded(new ExpandEvent(e));
			}
			break;
		}
		case SWT.FocusIn: {
		...
		case SWT.Resize: {
			((ControlListener) eventListener)
					.controlResized(new ControlEvent(e));
			break;
		}
		case SWT.Selection: {
			/* Fields set by Sash */

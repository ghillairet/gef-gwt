...
public class CoolBar extends Composite {
	...
	public CoolBar(Composite parent, int style) {
		...
		int[] events = new int[] { SWT.Dispose, SWT.MouseDown, SWT.MouseExit,
				SWT.MouseMove, SWT.MouseUp, SWT.MouseDoubleClick, SWT.Paint,
				SWT.Resize };
	...
	boolean insertItemIntoRow(CoolItem item, int rowIndex, int x_root) {
		...
		if (index > 0) {
			...
			left.setBounds(leftBounds.x, leftBounds.y, newWidth,
					leftBounds.height);
	...
	void createItem(CoolItem item, int index) {
		...
		if (items.length == 0) {
			...
			if (lastItem.ideal) {
				...
				lastItem.setBounds(bounds.x, bounds.y, bounds.width,
						bounds.height);
			}
			if (i == 0) {
				item.wrap = true;
	...
	void moveLeft(CoolItem item, int pixels) {
		...
		int leftWidth = Math.max(left.internalGetMinimumWidth(),
				leftBounds.width - pixels);
	...
	void moveRight(CoolItem item, int pixels) {
		...
		if (x - bounds.x > 0) {
			internalRedraw(bounds.x - CoolItem.MARGIN_WIDTH, bounds.y,
					damagedWidth, bounds.height);
		}
	}
	...
	void onDispose(Event event) {
		 ...
		 * the items array and reset the bounds of all the remaining cool items.
		 * Since the whole cool bar is being disposed, this is not necessary.
		 * For speed the inDispose flag is used to skip over this part of the
		 * item dispose.
		 */
		if (inDispose)
			return;
	...
	void onMouseMove(Event event) {
		...
		if (dragging != null) {
			...
			if (event.y < bounds.y) {
				...
				int distance = Math.min(mouseXOffset, bounds.x + itemXOffset)
						- event.x;
				if (distance > 0)
					moveLeft(dragging, distance);
			} else if (event.x > mouseXOffset) {
				int distance = event.x
						- Math.max(mouseXOffset, bounds.x + itemXOffset);
				if (distance > 0)
		...
		if (target == null) {
			...
			if (items[row].length > 1) {
				...
				if (bounds.width == maxSize) {
					/*
					 * The item is at its maximum width. It should be resized to
					 * its minimum width.
					 */
					int distance = bounds.width
							- target.internalGetMinimumWidth();
					if (index + 1 < items[row].length) {
						...
						/*
						 * There is no item to the right. Move the item all the
						 * way right.
						 */
					...
					/*
					 * The item is less than its preferredWidth. Resize to
					 * preferredWidth.
					 */
					...
					if (index + 1 < items[row].length) {
						...
						distance = target.preferredWidth
								- target.internalGetBounds().width;
					}
					if (distance > 0) {
					...
					for (int i = 0; i < items[row].length; i++) {
						if (i != index) {
							CoolItem item = items[row][i];
							item.requestedWidth = Math.max(
									item.internalGetMinimumWidth(),
	...
	void onPaint(Event event) {
		...
		if (items.length == 0)
			return;
		Color shadowColor = display
				.getSystemColor(SWT.COLOR_WIDGET_NORMAL_SHADOW);
		Color highlightColor = display
		...
		for (int row = 0; row < items.length; row++) {
			...
			for (int i = 0; i < items[row].length; i++) {
				...
				rect = fixRectangle(bounds.x, bounds.y, bounds.width,
						bounds.height);
				...
				/* Draw gripper. */
				if (!isLocked) {
					rect = fixRectangle(bounds.x, bounds.y,
							CoolItem.MINIMUM_WIDTH, bounds.height);
					if (!flat)
						nativeGripper = drawGripper(rect.x, rect.y, rect.width,
								rect.height, vertical);
					if (!nativeGripper) {
						int grabberTrim = 2;
						int grabberHeight = bounds.height - (2 * grabberTrim)
								- 1;
						gc.setForeground(shadowColor);
						rect = fixRectangle(bounds.x + CoolItem.MARGIN_WIDTH,
								bounds.y + grabberTrim, 2, grabberHeight);
						gc.drawRectangle(rect);
						gc.setForeground(highlightColor);
						rect = fixRectangle(bounds.x + CoolItem.MARGIN_WIDTH,
								bounds.y + grabberTrim + 1, bounds.x
										+ CoolItem.MARGIN_WIDTH, bounds.y
										+ grabberTrim + grabberHeight - 1);
						gc.drawLine(rect.x, rect.y, rect.width, rect.height);
						rect = fixRectangle(bounds.x + CoolItem.MARGIN_WIDTH,
								bounds.y + grabberTrim, bounds.x
										+ CoolItem.MARGIN_WIDTH + 1, bounds.y
										+ grabberTrim);
						gc.drawLine(rect.x, rect.y, rect.width, rect.height);
					}
				}
				...
				if (!flat && !nativeGripper && i != 0) {
					gc.setForeground(shadowColor);
					rect = fixRectangle(bounds.x, bounds.y, bounds.x, bounds.y
							+ bounds.height - 1);
					gc.drawLine(rect.x, rect.y, rect.width, rect.height);
					gc.setForeground(highlightColor);
					rect = fixRectangle(bounds.x + 1, bounds.y, bounds.x + 1,
	...
	boolean removeItemFromRow(CoolItem item, int rowIndex, boolean disposed) {
		...
		if (newLength > 0) {
			...
			System.arraycopy(items, rowIndex + 1, newRows, rowIndex,
					newRows.length - rowIndex);
			items = newRows;
			return true;
		}
		if (!disposed) {
		...
		for (int i = 0; i < newLength; i++) {
			newRowHeight = Math.max(newRowHeight,
					items[rowIndex][i].preferredHeight);
		}
		...
		for (int row = 0; row < items.length; row++) {
			...
			for (int i = 0; i < count; i++) {
				...
				if (!oldBounds.equals(newBounds)) {
					...
					/*
					 * Cases are in descending order from most area to redraw to
					 * least.
					 */
					if (oldBounds.y != newBounds.y) {
						...
						/*
						 * Draw from the bottom of the gripper to the bottom of
						 * the new area. (Bottom of the gripper is -3 from the
						 * bottom of the item).
						 */
					...
					internalRedraw(damage.x, damage.y, damage.width,
							damage.height);
				}
				x += newWidth;
			}
	...
	void wrapItems(int maxWidth) {
		...
		int start = 0;
		for (int row = 0; row < items.length; row++) {
			System.arraycopy(items[row], 0, itemsVisual, start,
					items[row].length);

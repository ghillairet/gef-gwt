...
/**
 * This class provides a default drag under effect during a drag and drop. The
 * current implementation does not provide any visual feedback.
 * 
 * <p>
 * The drop target effect has the same API as the <code>DropTargetAdapter</code>
 * so that it can provide custom visual feedback when a
 * <code>DropTargetEvent</code> occurs.
 * </p>
 * 
 * <p>
 * Classes that wish to provide their own drag under effect can extend the
 * <code>DropTargetEffect</code> and override any applicable methods in
 * <code>DropTargetAdapter</code> to display their own drag under effect.
 * </p>
 * 
 * <p>
...
public class DropTargetEffect extends DropTargetAdapter {
	...
	/**
	 * Creates a new <code>DropTargetEffect</code> to handle the drag under
	 * effect on the specified <code>Control</code>.
	 * 
	 * @param control
	...
	/**
	 * Returns the Control which is registered for this DropTargetEffect. This
	 * is the control over which the user positions the cursor to drop the data.
	 * 
	...
	/**
	 * Returns the item at the given x-y coordinate in the receiver or null if
	 * no such item exists. The x-y coordinate is in the display relative
	 * coordinates.
	 * 
	 * @param x
	 *            the x coordinate used to locate the item
	 * @param y
	 *            the y coordinate used to locate the item
	 * @return the item at the given x-y coordinate, or null if the coordinate
	 *         is not in a selectable item
	 */
	...
	TreeItem nextItem(Tree tree, TreeItem item) {
		...
		int index = parentItem == null ? tree.indexOf(childItem) : parentItem
				.indexOf(childItem);
		int count = parentItem == null ? tree.getItemCount() : parentItem
				.getItemCount();
		while (true) {
			if (index + 1 < count)
				return parentItem == null ? tree.getItem(index + 1)
						: parentItem.getItem(index + 1);
			if (parentItem == null)
				return null;
			childItem = parentItem;
			parentItem = childItem.getParentItem();
			index = parentItem == null ? tree.indexOf(childItem) : parentItem
					.indexOf(childItem);
			count = parentItem == null ? tree.getItemCount() : parentItem
					.getItemCount();
		}
	}
	...
	TreeItem previousItem(Tree tree, TreeItem item) {
		...
		if (index == 0)
			return parentItem;
		TreeItem nextItem = parentItem == null ? tree.getItem(index - 1)
				: parentItem.getItem(index - 1);

...
import org.eclipse.swt.internal.gwt.GtkSelectionData;
...
public class Clipboard {
	 ...
	 * controlled points. This method enforces this rule unless it is
	 * overridden.
	 * </p>
	 * <p>
	...
	public Object getContents(Transfer transfer, int clipboards) {
		...
		for (int i = 0; i < typeIds.length; i++) {
			if ((clipboards & DND.CLIPBOARD) != 0) {
				selection_data = gtk_clipboard_wait_for_contents(GTKCLIPBOARD,
						typeIds[i]);
				/*
				 * This call to gdk_threads_leave() is a temporary work around
				 * to avoid deadlocks when gdk_threads_init() is called by
				 * native code outside of SWT (i.e AWT, etc). It ensures that
				 * the current thread leaves the GTK lock acquired by the
				 * function above.
				 */
		...
		// OS.memmove(gtkSelectionData, selection_data,
		// GtkSelectionData.sizeof);
	...
	public TransferData[] getAvailableTypes(int clipboards) {
		...
		if ((clipboards & DND.SELECTION_CLIPBOARD) != 0) {
			...
			int offset = 0;
			if (result != null) {
				TransferData[] newResult = new TransferData[result.length
						+ types.length];
	...
	long /* int */gtk_clipboard_wait_for_contents(long /* int */clipboard,
			long /* int */target) {
		return 0;
	}
}

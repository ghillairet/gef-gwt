...
public final class CommandManager extends HandleObjectManager implements
	 ...
	 * @return a <code>String</code> representing <code>escapedText</code> with
	 *         any escaped characters replaced by their literal values
	...
	/**
	 * The help context identifiers ({@link String}) for a handler (
	 * {@link IHandler}). This map may be empty, but it is never
	 * <code>null</code>. Entries are removed if all strong references to the
	 * handler are removed.
	 * 
	 * @since 3.2
	 */
	...
	/**
	 * The map of parameter type identifiers (<code>String</code>) to parameter
	 * types ( <code>ParameterType</code>). This collection may be empty, but it
	 ...
	 * 
	 * @see
	 * org.eclipse.core.commands.ICategoryListener#categoryChanged(org.eclipse
	 * .core.commands.CategoryEvent)
	 */
	 ...
	 * 
	 * @see
	 * org.eclipse.commands.ICommandListener#commandChanged(org.eclipse.commands
	 * .CommandEvent)
	 */
	 ...
	 * <code>serializedParameterizedCommand</code> must use the format returned
	 * by {@link ParameterizedCommand#serialize()} and described in the Javadoc
	 * for that method.
	 * </p>
	 * <p>
	 * If a parameter id encoded in the
	 * <code>serializedParameterizedCommand</code> does not exist in the encoded
	 * command, that parameter id and value are ignored. A given parameter id
	 * should not be used more than once in
	 * <code>serializedParameterizedCommand</code>. This will not result in an
	 * exception, but in this case the value of the parameter when the command
	 * is executed is unspecified.
	 * </p>
	 * <p>
	 * This method will never return <code>null</code>, however it may throw an
	 * exception if there is a problem processing the serialization string or
	 ...
	 * 
	 * @param categoryId
	 *            The identifier to find; must not be <code>null</code>. If the
	 *            category is <code>null</code>, then a category suitable for
	 ...
	 * 
	 * @param commandId
	 *            The identifier to find; must not be <code>null</code> and must
	 *            not be zero-length.
	 ...
	 *            A map of command identifiers (<code>String</code>) to handlers
	 *            (<code>IHandler</code>). This map may be <code>null</code> if
	 *            all handlers should be cleared. Similarly, if the map is
	 *            empty, then all commands will become unhandled.
	 */
	 ...
	 *         <code>escapedText</code>, or <code>-1</code> if the character
	 ...
	 * <p>
	 * <b>Note:</b> This supports bridging actions to the command framework, and
	 * should not be used outside the framework.
	 * </p>
	 * 
	 * @param commandId

...
public abstract class AbstractTreeViewer extends ColumnViewer {
	 ...
	 * viewer's input is changed (that is, by <code>setInput</code>). A value of
	 * 0 means that auto-expand is off.
	 * 
	...
	private void createAddedElements(Widget widget, Object[] elements) {
		...
		// Count of elements we have added. See bug 205700 for why this is
		// needed.
		int newItems = 0;
		...
		elementloop: for (int i = 0; i < elements.length; i++) {
			...
			indexInItems = insertionPosition(items, comparator, indexInItems,
					element, parentPath);
			if (indexInItems == items.length) {
				...
				while (insertionIndexInItems < items.length
					...
					if (items[insertionIndexInItems].getData().equals(element)) {
						...
						internalRefresh(element);
						// Do not create a new item - continue with the next
						// element.
				...
				if (insertionIndexInItems == items.length) {
					...
					// InsertionIndexInItems is the index in the original array.
					// We
					// need to correct by the number of new items we have
					// created. See bug 205700.
					createTreeItem(widget, element, insertionIndexInItems
							+ newItems);
					newItems++;
				}
			}
	 ...
	 * @param parentPath
	 *            the tree path for the element's parent or <code>null</code> if
	 *            the element is a root element or the sorter is not a
	 ...
	 * 
	 * @see
	 * org.eclipse.jface.viewers.StructuredViewer#getSortedChildren(java.lang
	 * .Object)
	 */
	protected Object[] getSortedChildren(Object parentElementOrTreePath) {
		...
		if (parentElementOrTreePath != null
			...
			Object[] clone = new Object[result.length];
	 ...
	 *            the element or tree path
	 * @param level
	 *            non-negative level, or <code>ALL_LEVELS</code> to collapse all
	 *            levels of the tree
	 */
	 ...
	 * 
	 * @see
	 * org.eclipse.jface.viewers.StructuredViewer#handleDoubleSelect(org.eclipse
	...
	/**
	 * Initializes the tree with root items, expanding to the appropriate level
	 * if necessary.
	 * 
	 * @param tree
	 *            the tree control
	 ...
	 * 
	 * @param result
	 *            a list (element type: <code>Item</code>) into which to collect
	 *            the elements
	 * @param widget
	 *            the widget
	 */
	 ...
	 *            the element
	 * @param expand
	 *            <code>true</code> if all nodes on the path should be expanded,
	...
	private void updateChildren(Widget widget, Object parent,
		...
		if (widget instanceof Item) {
			...
			if (!getExpanded(ti)) {
				...
				// The following code is being executed if children were not
				// pruned.
			...
			while (numItemsToDispose > 0 && i < items.length) {
				...
				if (data == null || !children.containsKey(data)) {
					...
					if (i + 1 < items.length) {
						...
						System.arraycopy(items, i + 1, items, i, items.length
								- (i + 1));
					}
					numItemsToDispose--;
				} else {
			...
			if (widget instanceof Item) {
				...
				if (getExpanded(item)
					...
					if (element instanceof TreePath)
						parentElement = ((TreePath) parentElement)
								.getLastSegment();
	...
	/**
	 * This implementation of {@link #createViewerEditor()} returns null to
	 * ensure API backwards compatibility. Subclasses should override.
	 * 
	 * @since 3.3
	 */
	 ...
	 * <p>
	 * <b>Subclasses should overwrite this method, which has a default
	 * implementation (returning 0) for API backwards compatility reasons</b>
	 * </p>
	 * 
	...
	/**
	 * Returns true if the given object is either the input or an empty tree
	 * path.
	 * 
	 * @param elementOrTreePath
	 *            an element which could either be the viewer's input, or a tree
	 *            path
	 * 
	 * @return <code>true</code> if the given object is either the input or an
	 *         empty tree path, <code>false</code> otherwise.
	 * @since 3.3
	 */
	final protected boolean internalIsInputOrEmptyPath(

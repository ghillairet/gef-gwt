...
public final class ExternalActionManager {
	 ...
	 * @since 3.1
	 */
	public static class CommandCallback implements IBindingManagerListener,
			IBindingManagerCallback, IExecuteCallback {
		 ...
		 * notification. This is a map of command identifiers (
		 * <code>String</code>) to listeners (
		 * <code>IPropertyChangeListener</code> or <code>ListenerList</code> of
				...
				while (listenerItr.hasNext()) {
					...
					if (event.isActiveBindingsChangedFor(parameterizedCommand)) {
						...
						PropertyChangeEvent propertyChangeEvent = new PropertyChangeEvent(
								event.getManager(), IAction.TEXT, null, null);
						if (value instanceof ListenerList) {
							Object[] listeners = ((ListenerList) value)
									.getListeners();
		 ...
		 *            requested. This argument may be <code>null</code>. It is
		 *            assumed that the command has no parameters.
			...
			if (commandId != null) {
				...
				if (!command.isDefined()
					...
					IStatus status = new Status(IStatus.ERROR,
							...
							0, "externalactionmanager", new Exception());
					...
					command.addCommandListener(new ICommandListener() {
						 ...
						 * 
						 * @see
						 * org.eclipse.ui.commands.ICommandListener#commandChanged
		...
		public void postExecuteSuccess(IAction action, Object returnValue) {
			...
			commandManager.firePostExecuteSuccess(actionDefinitionId,
					returnValue);
		}
		...
		public void postExecuteFailure(IAction action,
			...
			commandManager
					.firePostExecuteFailure(actionDefinitionId, exception);
		}
	...
	public static interface IExecuteCallback {
		 ...
		 * 
		 * @param action
		 *            The action contribution that caused the exception, never
		 *            <code>null</code>.
		 * @param exception
		 *            The <code>NotEnabledException</code>, never
		 *            <code>null</code>.
		 */
		public void notEnabled(IAction action, NotEnabledException exception);
		 ...
		 *            <code>null</code>.
		 * @param exception
		 *            The <code>NotDefinedException</code>, never
		 *            <code>null</code>.
		 */
		public void notDefined(IAction action, NotDefinedException exception);
		 ...
		 * 
		 * @param action
		 *            The action contribution that requires an execution event
		 *            to be fired. Cannot be <code>null</code>.
		 * @param e
		...
		/**
		 * Creates an <code>ExecutionException</code> when the action returned a
		 * failure.
		 * 
		 * @param action
		 *            The action contribution that caused the exception, never
		 *            <code>null</code>.
		 * @param exception
		 *            The <code>ExecutionException</code>, never
		 *            <code>null</code>.
		 */
		public void postExecuteFailure(IAction action,
				ExecutionException exception);
	...
	public static interface ICallback {
		 ...
		 * </p>
		 * <p>
		 * Has no effect if an identical listener has already been added for the
	 ...
	 *         action contribution items. This value may be <code>null</code> if
	 ...
	 *            <code>null</code> if the default is acceptable (i.e., no extra
	 *            information will provided to actions).
	 */

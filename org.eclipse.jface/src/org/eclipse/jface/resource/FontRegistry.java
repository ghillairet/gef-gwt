...
/**
 * A font registry maintains a mapping between symbolic font names and SWT
 * fonts.
 * <p>
 * A font registry owns all of the font objects registered with it, and
 * automatically disposes of them when the SWT Display that creates the fonts is
 * disposed. Because of this, clients do not need to (indeed, must not attempt
 * to) dispose of font objects themselves.
 * </p>
 * <p>
 * A special constructor is provided for populating a font registry from a
 * property files using the standard Java resource bundle mechanism.
 * </p>
 * <p>
 * Methods are provided for registering listeners that will be kept apprised of
 * changes to list of registered fonts.
 * </p>
 * <p>
...
public class FontRegistry extends ResourceRegistry {
	...
	/**
	 * FontRecord is a private helper class that holds onto a font and can be
	 * used to generate its bold and italic version.
	 */
	private class FontRecord {
		...
		/**
		 * Create a new instance of the receiver based on the plain font and the
		 * data for it.
		 * 
		 * @param plainFont
		 *            The base looked up font.
		 * @param data
		 *            The data used to look it up.
		 */
		...
		/**
		 * Return the bold Font. Create a bold version of the base font to get
		 * it.
		 * 
		 * @return Font
		 */
		...
		/**
		 * Get a version of the base font data with the specified style.
		 * 
		 * @param style
		 *            the new style
		 * @return the font data with the style
		 *         {@link FontData#FontData(String, int, int)}
		 * @see SWT#ITALIC
		 * @see SWT#NORMAL
		 * @see SWT#BOLD
		...
		/**
		 * Return the italic Font. Create an italic version of the base font to
		 * get it.
		 * 
		 * @return Font
		 */
		...
		/**
		 * Add any fonts that were allocated for this record to the stale fonts.
		 * Anything that matches the default font will be skipped.
		 * 
		 * @param defaultFont
		 *            The system default.
		 */
	...
	/**
	 * Table of known fonts, keyed by symbolic font name (key type:
	 * <code>String</code>, value type: <code>FontRecord</code>.
	 */
	private Map stringToFontRecord = new HashMap(7);

	/**
	 * Table of known font data, keyed by symbolic font name (key type:
	...
	/**
	 * Collection of Fonts that are now stale to be disposed when it is safe to
	 * do so (i.e. on shutdown).
	 * 
	 * @see List
	 */
	 ...
	 * is converted to lowercase, purged of whitespace, and appended as suffix
	 * (separated by an underscore <code>'_'</code>) to the given location
	 * string to yield the base name of a resource bundle acceptable to
	 * <code></code>. The standard Java resource bundle mechanism is then used
	 * to locate and open the appropriate properties file, taking into account
	 ...
	 * <code>.<it>n</it></code> suffix to standard font names on the right. The
	 * suffix indicated order of preference: "<code>.0</code>" indicates the
	 * first choice, "<code>.1</code>" indicates the second choice, and so on.
	 * </p>
	 ...
	 * @since 2.1
	 */
	public FontRegistry(String location, Object/* ClassLoader */loader)
	 ...
	 * <p>
	 * This method should only be called from the UI thread. If you are not on
	 * the UI thread then wrap the call with a
	 * <code>PlatformUI.getWorkbench().getDisplay().synchExec()</code> in order
	 * to guarantee the correct result. Failure to do this may result in an
	 * {@link SWTException} being thrown.
	 * </p>
	 * 
	 * @param location
	...
	public FontRegistry(String location) throws MissingResourceException {
		// FIXE:
		// this(location,
		// WorkbenchPlugin.getDefault().getDescriptor().getPluginClassLoader());
		this(location, null);
	}

	/**
	 * Read the resource bundle at location. Look for a file with the extension
	 * _os_ws first, then _os then just the name.
	 * 
	 * @param location
	...
	/**
	 * Find the first valid fontData in the provided list. If none are valid
	 * return the first one regardless. If the list is empty return
	 * <code>null</code>.
	 * 
	 * @param fonts
	 *            list of fonts
	 ...
	 * @deprecated use filterData in order to preserve multiple entry fonts on
	 *             Motif
	 */
	...
	/**
	 * Removes from the list all fonts that do not exist in this system. If none
	 * are valid, return the first irregardless. If the list is empty return
	 * <code>null</code>.
	 * 
	 * @param fonts
	 *            the fonts to check
	...
	/**
	 * Creates a new font with the given font datas or <code>null</code> if
	 * there is no data.
	 * 
	...
	/**
	 * Calculates the default font and returns the result. This method creates a
	 * font that must be disposed.
	 */
	...
	/**
	 * Returns the font descriptor for the font with the given symbolic font
	 * name. Returns the default font if there is no special value associated
	 * with that name
	 * 
	 * @param symbolicName
	 *            symbolic font name
	...
	private FontRecord defaultFontRecord() {
		...
		if (record == null) {
			...
			record = createFont(JFaceResources.DEFAULT_FONT,
	...
	/**
	 * Returns the font associated with the given symbolic font name. Returns
	 * the default font if there is no special value associated with that name.
	 * <p>
	 * This method should only be called from the UI thread. If you are not on
	 * the UI thread then wrap the call with a
	 * <code>PlatformUI.getWorkbench().getDisplay().synchExec()</code> in order
	 ...
	 * 
	 * @see
	 * org.eclipse.jface.resource.ResourceRegistry#hasValueFor(java.lang.String)
	 */
	...
	/**
	 * Adds (or replaces) a font to this font registry under the given symbolic
	 * name.
	 * <p>
	 * A property change event is reported whenever the mapping from a symbolic
	 * name to a font changes. The source of the event is this registry; the
	 * property name is the symbolic font name.
	 * </p>
	 * 
	 * @param symbolicName
	 ...
	 *            an Array of FontData
	 * @param update
	 *            - fire a font mapping changed if true. False if this method is
	 *            called from the get method as no setting has changed.
	 */
	...
	/**
	 * Reads the resource bundle. This puts FontData[] objects in the mapping
	 * table. These will lazily be turned into real Font objects when requested.
	 */

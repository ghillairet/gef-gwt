...
public final class TraverseEvent extends KeyEvent {
	 ...
	 * </p>
	 * 
	 * Setting this field will change the type of traversal. For example,
	 * setting the detail to <code>TRAVERSE_NONE</code> causes no traversal
	 * action to be taken.
	 * 
	 * When used in conjunction with the <code>doit</code> field, the traversal
	 * detail field can be useful when overriding the default traversal

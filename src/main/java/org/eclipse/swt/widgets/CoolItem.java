...
public class CoolItem extends Item {
	...
	private static final int CHEVRON_IMAGE_WIDTH = 8; // Width to draw the
														// double arrow
	...
	public void dispose() {
		...
		/*
		 * Must call parent.destroyItem() before super.dispose(), since it needs
		 * to query the bounds to properly remove the item.
		 */
		...
		/*
		 * Although the parent for the chevron is the CoolBar (CoolItem can not
		 * be the parent) it has to be disposed with the item
		 */
	...
	public Rectangle getBounds() {
		checkWidget();
		return parent.fixRectangle(itemBounds.x, itemBounds.y,
	...
	public void setControl(Control control) {
		...
		if (control != null) {
			...
			if ((style & SWT.DROP_DOWN) != 0
					&& itemBounds.width < preferredWidth) {
				controlWidth -= CHEVRON_IMAGE_WIDTH + CHEVRON_HORIZONTAL_TRIM
						+ CHEVRON_LEFT_MARGIN;
			}
		...
		if (control != null) {
			...
			if ((style & SWT.DROP_DOWN) != 0 && width < preferredWidth) {
				...
				if (currentImageHeight != imageHeight) {
					Image image = createArrowImage(CHEVRON_IMAGE_WIDTH,
							imageHeight);
				...
				chevron.setBounds(parent.fixRectangle(itemBounds.x + width
						...
						- CHEVRON_HORIZONTAL_TRIM, itemBounds.y,
						CHEVRON_IMAGE_WIDTH + CHEVRON_HORIZONTAL_TRIM, height));

...
public class TaskBar extends Widget {
	...
	/**
	 * Returns the <code>TaskItem</code> for the given <code>Shell</code> or the
	 * <code>TaskItem</code> for the application if the <code>Shell</code>
	 * parameter is <code>null</code>. If the requested item is not supported by
	 * the platform it returns <code>null</code>.
	 * 
	 * @param shell
	 *            the shell for which the task item is requested, or null to
	 *            request the application item
	...
	/**
	 * Returns an array of <code>TaskItem</code>s which are the items in the
	 * receiver.
	 * <p>

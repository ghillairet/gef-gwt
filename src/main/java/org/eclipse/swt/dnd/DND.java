...
/**
 * Class DND contains all the constants used in defining a DragSource or a
 * DropTarget.
 * 
...
public class DND {
	...
	/**
	 * The transfer mechanism for data that is being cut and then pasted or
	 * copied and then pasted (value is 1).
	 * 
	 * @see Clipboard
	 * 
	 * @since 3.1
	...
	/**
	 * The transfer mechanism for clients that use the selection mechanism
	 * (value is 2).
	 * 
	 * @see Clipboard
	 * 
	...
	/**
	 * Drag and Drop Operation: a copy of the data in the drag source is added
	 * to the drop target (value is 1 &lt;&lt; 0).
	 */
	public final static int DROP_COPY = 1 << 0;

	/**
	 * Drag and Drop Operation: a copy of the data is added to the drop target
	 * and the original data is removed from the drag source (value is 1
	 * &lt;&lt; 1).
	 */
	public final static int DROP_MOVE = 1 << 1;

	/**
	 * Drag and Drop Operation: the drop target makes a link to the data in the
	 * drag source (value is 1 &lt;&lt; 2).
	 */
	public final static int DROP_LINK = 1 << 2;

	/**
	 * Drag and Drop Operation: the drop target moves the data and the drag
	 * source removes any references to the data and updates its display. This
	 * is not available on all platforms and is only used when a non-SWT
	 * application is the drop target. In this case, the SWT drag source is
	 * informed in the dragFinished event that the drop target has moved the
	 * data. (value is 1 &lt;&lt; 3).
	 * 
	 ...
	 * dragOperationChanged, if no modifier keys are pressed, the operation is
	 * set to DROP_DEFAULT. The application can choose what the default
	 * operation should be by setting a new value in the operation field. If no
	 * value is choosen, the default operation for the platform will be selected
	 * (value is 1 &lt;&lt; 4).
	 * 
	...
	/**
	 * DragSource Event: the drop has successfully completed or has been
	 * terminated (such as hitting the ESC key); perform cleanup such as
	 * removing data on a move operation (value is 2000).
	 */
	...
	/**
	 * DropTarget Event: the cursor has left the drop target boundaries OR the
	 * drop operation has been cancelled (such as by hitting ECS) OR the drop is
	 * about to happen (user has released the mouse button over this target)
	 * (value is 2003).
	 */
	...
	/**
	 * DropTarget Event: the operation being performed has changed usually due
	 * to the user changing the selected modifier keys while dragging (value is
	 * 2005).
	 */
	...
	/**
	 * DropTarget Event: the drop target is given a last chance to modify the
	 * drop (value is 2007).
	 */
	...
	/**
	 * DropTarget drag under effect: The item under the cursor is selected;
	 * applies to tables and trees (value is 1).
	 */
	public static final int FEEDBACK_SELECT = 1;

	/**
	 * DropTarget drag under effect: An insertion mark is shown before the item
	 * under the cursor; applies to tables and trees (value is 2).
	 */
	public static final int FEEDBACK_INSERT_BEFORE = 2;

	/**
	 * DropTarget drag under effect: An insertion mark is shown after the item
	 * under the cursor; applies to tables and trees (value is 4).
	 */
	public static final int FEEDBACK_INSERT_AFTER = 4;

	/**
	 * DropTarget drag under effect: The widget is scrolled up or down to allow
	 * the user to drop on items that are not currently visible; applies to
	 * tables and trees (value is 8).
	 */
	public static final int FEEDBACK_SCROLL = 8;

	/**
	 * DropTarget drag under effect: The item currently under the cursor is
	 * expanded to allow the user to select a drop target from a sub item;
	 * applies to trees (value is 16).
	 */
	...
	/**
	 * DropTarget Key: The string constant for looking up the drop target for a
	 * control using <code>getData(String)</code>. When a drop target is created
	 * for a control, it is stored as a property in the control using
	 * <code>setData(String, Object)</code>.
	 * 
	 * @since 3.4
	 */
	...
	/**
	 * DragSource Key: The string constant for looking up the drag source for a
	 * control using <code>getData(String)</code>. When a drag source is created
	 * for a control, it is stored as a property in the control using
	 * <code>setData(String, Object)</code>.
	 * 
	 * @since 3.4
	 ...
	 * <code>hresult</code> argument should be either 0, or the platform
	 * specific error code.
	 * <p>

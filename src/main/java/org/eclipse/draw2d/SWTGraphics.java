...
public class SWTGraphics extends Graphics {
	...
	static class RectangleClipping implements Clipping {
		...
		public void intersect(int left, int top, final int right,
				final int bottom) {
			// SzI:
			int l = (int) Math.ceil(this.left);
			int t = (int) Math.ceil(this.top);
			this.left = Math.max(Math.min(this.right, this.left),
					Math.min(right, left));
			this.right = Math.min(Math.max(l, this.right),
					Math.max(left, right));
			this.top = Math.max(Math.min(this.bottom, this.top),
					Math.min(bottom, top));
			this.bottom = Math.min(Math.max(t, this.bottom),
					Math.max(top, bottom));
		...
		public void setOn(GC gc, int translateX, int translateY) {
			...
			gc.setClipping(xInt + translateX, yInt + translateY,
					(int) Math.ceil(right) - xInt, (int) Math.ceil(bottom)
							- yInt);
			// gc.setClipping(xInt, yInt, (int) Math.ceil(right) - xInt,
	...
	static class State extends LazyState implements Cloneable {
		...
		public State cloneState() {
			...
			if (affineMatrix != null) {
				float[] amCopy = new float[this.affineMatrix.length];
				System.arraycopy(this.affineMatrix, 0, amCopy, 0,
						this.affineMatrix.length);
				clone.affineMatrix = amCopy;
			} else {
				clone.affineMatrix = null;
			}

			/**
			 * FIXME: need to create a clone of the non-primitive types
			 * 
			 * @return
			 */
			clone.fgPattern = this.fgPattern;
			clone.fgColor = this.fgColor;
			clone.bgColor = this.bgColor;
			clone.font = this.font;
			clone.bgPattern = this.bgPattern;
			clone.graphicHints = this.graphicHints;
			clone.relativeClip = this.relativeClip.getCopy();

			clone.lineAttributes = SWTGraphics.clone(this.lineAttributes);
			return clone;
		}
	...
	protected final void checkFill() {
		if (!currentState.bgColor.equals(appliedState.bgColor)
				&& currentState.bgPattern == null && useGradient == false) {
	...
	protected final void checkText() {
		checkPaint();
		if (appliedState.font.equals(currentState.font) == false) {
	...
	public void drawString(String s, int x, int y) {
		checkText();
		Dimension textDimension = FigureUtilities.getTextExtents(s, getFont());
	...
	public void drawText(String s, int x, int y) {
		checkText();
		// Dimension textDimension = FigureUtilities.getTextExtents(s,
		// getFont());
		// y = y + textDimension.height;
	...
	public float[] getLineDash() {
		...
		float[] dash = currentState.lineAttributes.dash;
		float[] copy = new float[dash.length];
		System.arraycopy(dash, 0, copy, 0, dash.length);
		return copy;
	}
	...
	public void pushState() {
		...
		if (stack.size() > stackPointer) {
			...
			stack.add(currentState.cloneState());
		}
		sharedClipping = true;
		stackPointer++;
	}
	...
	public void setLineDash(float[] value) {
		if (value != null) {
			...
			float[] valueCopy = new float[value.length];
			System.arraycopy(value, 0, valueCopy, 0, value.length);
			currentState.lineAttributes.dash = valueCopy;
	...
	public void setLineGradient(int i, int y, Color bgc, int j, int height,
			Color transformColor) {
		useGradient = true;
		gc.setFillStyle(i, y, bgc, j, height, transformColor);
	}
}
...
/* package */class LayoutGenerator {
	...
	private static final GridDataFactory nonWrappingLabelData = GridDataFactory
			.fillDefaults().align(SWT.BEGINNING, SWT.CENTER).grab(false, false);
	 ...
	 * 
	 * @param control
	 *            the control the factory is search for
	...
	public static GridDataFactory defaultsFor(Control control) {
		...
		if (control instanceof Scrollable) {
			...
			if (scrollable instanceof Composite) {
				...
				if (theLayout instanceof GridLayout) {
					...
					return GridDataFactory.fillDefaults().grab(
							growsHorizontally, growsVertically);
				}
			}
		}
		...
		boolean containsText = hasMethod(control,
				"setText", new Class[] { String.class }); //$NON-NLS-1$
		...
		boolean userEditable = !hasStyle(control, SWT.READ_ONLY)
				&& containsText
				&& hasMethod(
						control,
						"addModifyListener", new Class[] { ModifyListener.class }); //$NON-NLS-1$
		...
		return GridDataFactory.fillDefaults().grab(grabHorizontal, vScroll)
				.align(SWT.FILL, vAlign)
				.hint(hHint, vScroll ? defaultSize.y : SWT.DEFAULT);
	}

	private static boolean hasMethod(Control control, String name,

...
public class ApplicationWindow extends Window implements IRunnableContext {
	...
	/**
	 * Internal application window layout class. This vertical layout supports a
	 * tool bar area (fixed size), a separator line, the content area (variable
	 * size), and a status line (fixed size).
	 */
	/* package */class ApplicationWindowLayout extends Layout {
		...
		protected void layout(Composite composite, boolean flushCache) {
			...
			// Lay out the separator, the tool bar control, the cool bar
			// control, the status line, and the page composite.
			// The following code assumes that the page composite is the last
			// child, and that there are no unexpected other controls.
	...
	/**
	 * Create an application window instance, whose shell will be created under
	 * the given parent shell. Note that the window will have no visual
	 * representation (no widgets) until it is told to open. By default,
	 * <code>open</code> does not block.
	 * 
	 * @param parentShell
	...
	/**
	 * Configures this window to have a menu bar. Does nothing if it already has
	 * one. This method must be called before this window's shell is created.
	 */
	...
	/**
	 * Configures this window to have a status line. Does nothing if it already
	 * has one. This method must be called before this window's shell is
	 * created.
	 */
	...
	/**
	 * Configures this window to have a tool bar. Does nothing if it already has
	 * one. This method must be called before this window's shell is created.
	 * 
	 * @param style
	...
	/**
	 * Configures this window to have a cool bar. Does nothing if it already has
	 * one. This method must be called before this window's shell is created.
	 * 
	 * @param style
	 *            the cool bar style
	 ...
	 * Returns a new tool bar manager for the window.
	 * <p>
	 * By default this method calls <code>createToolBarManager</code>.
	 * Subclasses may override this method to provide an alternative
	 * implementation for the tool bar manager.
	 * </p>
	 * 
	 * @param style
	 *            swt style bits used to create the Toolbar
	 ...
	 * <p>
	 * The default implementation of this framework method obtains the symbolic
	 * name of the font from the <code>getSymbolicFontName</code> framework
	 * method and retrieves this font from JFace's font registry using
	 * <code>JFaceResources.getFont</code>. Subclasses may override to use a
	 * different registry, etc.
	 * </p>
	 * 
	 ...
	 * 
	 * @return the menu bar manager, or <code>null</code> if this window does
	 *         not have a menu bar
	 * @see #addMenuBar()
	 */
	...
	/**
	 * Returns the symbolic font name of the font to be used to display text in
	 * this window. This is not recommended and is included for backwards
	 * compatability. It is recommended to use the default font provided by SWT
	 * (that is, do not set the font).
	 * 
	 ...
	 * 
	 * @return the tool bar manager, or <code>null</code> if this window does
	 *         not have a tool bar
	 * @see #addToolBar(int)
	 */
	...
	/**
	 * Sets or clears the message displayed in this window's status line (if it
	 * has one). This method has no effect if the window does not have a status
	 * line.
	 * 
	 * @param message
	 *            the status message, or <code>null</code> to clear it
	 */

...
public class MenuManager extends ContributionManager implements IMenuManager {
	...
	/**
	 * List of registered menu listeners (element type:
	 * <code>IMenuListener</code>).
	 */
	...
	/**
	 * Creates and returns an SWT context menu control for this menu, and
	 * installs all registered contributions. Does not create a new control if
	 * one already exists.
	 * <p>
	 ...
	 * does not clean up references between this menu manager and its associated
	 ...
	 * 
	 * @see
	 * org.eclipse.jface.action.IMenuManager#findUsingPath(java.lang.String)
	 */
	...
	/**
	 * Returns whether this menu should be enabled or not. Used to enable the
	 * menu item containing this menu when it is realized as a sub-menu.
	 * <p>
	...
	public void markDirty() {
		...
		// That is, it's possible to have a dirty sub-menu under a non-dirty
		// parent menu
		// even after the parent menu has been updated.
		// If items are added/removed in the sub-menu, we still need to
		// propagate the dirty flag up,
		// even if the sub-menu is already dirty, since the result of
		// isVisible() may change
		...
		if (isDirty() || force) {
			if (menuExist()) {
				...
				for (Iterator e = clean.iterator(); e.hasNext();) {
					...
					// (see MenuManager). So if force is true we do it again.
					// But
					// we can't set force to false since then information for
					// the
			...
			// I am not dirty. Check if I must recursivly walk down the
			// hierarchy.
			if (recursive) {
				IContributionItem[] items = getItems();
				for (int i = 0; i < items.length; ++i) {
				...
				if (text != null) {
					...
					if (callback != null) {
						...
						if (index >= 0 && index < text.length() - 1) {
							...
							if (callback
									.isAcceleratorInUse(SWT.ALT | character)
									&& isTopLevelMenu()) {
								if (index == 0) {
	...
	/**
	 * Updates the menu item for this sub menu. The menu item is disabled if
	 * this sub menu is empty. Does nothing if this menu is not a submenu.
	 */
	private void updateMenuItem() {
		 ...
		 * IContributionItem items[] = getItems(); boolean enabled = false; for
		 * (int i = 0; i < items.length; i++) { IContributionItem item =
		 * items[i]; enabled = item.isEnabled(); if(enabled) break; } //
		 * Workaround for 1GDDCN2: SWT:Linux - MenuItem.setEnabled() always
		 * causes a redraw if (menuItem.getEnabled() != enabled)

 ...
 * the command via its control, the action's <code>run</code> method is invoked
 * to do the real work.
 * <p>
...
public interface IAction {
	 ...
	 * not specified yet. By default, the action will assume a push button
	...
	/**
	 * Action style constant (value <code>2</code>) indicating action is a check
	 * box (or a toggle button).
	 */
	...
	/**
	 * Action style constant (value <code>8</code>) indicating action is a radio
	 * button.
	 * 
	 * @since 2.1
	 */
	 ...
	 * <code>"description"</code>). Typically the description is shown as a
	...
	/**
	 * Returns the accelerator keycode for this action. The result is the
	 * bit-wise OR of zero or more modifier masks and a key, as explained in
	 ...
	 * when the user hovers over the item. This method is associated with the
	...
	/**
	 * Returns the checked status of this action. Applicable only if the style
	 * is <code>AS_CHECK_BOX</code> or <code>AS_RADIO_BUTTON</code>.
	 * <p>
	...
	/**
	 * Removes the given listener from this action. Has no effect if an
	 ...
	 * 
	 * @param text
	 *            the description, or <code>null</code> to clear the description
	 * @see #DESCRIPTION
	 */
	 ...
	 * active; triggering it will end up invoking this action's <code>run</code>
	 * method.
	 * </p>
	 * <p>
	 ...
	 * <p>
	 * Hover images will be used on platforms that support changing the image
	 * when the user hovers over the item.Fires a property change event for the
	 * <code>IMAGE</code> property if the image actually changes as a
	 * consequence.
	 * </p>
	 * 
	 ...
	 * action is created. It should not be modified once the action is part of
	 * an action contribution item.
	 * 
	 * @param id
	 *            the action id
	 * 
	 ...
	 * <p>
	 * An accelerator specification may follow the actual text, separated from
	 * it by an '@' or a '\t' character. An accelerator specification consists
	 * of zero or more modifier tokens followed by a key code token. The tokens
	...
	/**
	 * <p>
	 * Sets the accelerator keycode that this action maps to. This is a bitwise
	 * OR of zero or more SWT key modifier masks (i.e. SWT.CTRL or SWT.ALT) and
	 * a character code. For example, for Ctrl+Z, use
	 * <code>SWT.CTRL | 'Z'</code>. Use 0 for no accelerator.
	 * </p>
	 * <p>
	 * This method should no longer be used for actions in the Eclipse
	 * workbench. <code>IWorkbenchCommandSupport</code> and
	 ...
	 * definition id. The definition id should be used instead -- referring to
	 * the command in the workbench from which the key binding should be
	 * retrieved.
	 * </p>
	 * 
	 * @param keycode

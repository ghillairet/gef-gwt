...
public interface IEclipsePreferences extends Preferences {
	...
	public final class NodeChangeEvent extends EventObject {
		...
		/**
		 * Return the parent node for this event. This is the parent of the node
		...
		/**
		 * Return the child node for this event. This is the node which was
		 * added or removed.
		 * <p>
		 * Note: The child node may have been removed as a result of the bundle
		 * supplying its implementation being un-installed. In this case the
		 * only method which can safely be called on the child is #name().
		 * </p>
		 * 
	...
	public interface INodeChangeListener {
		 ...
		 * 
		 * @param event
		 *            an event specifying the details about the new node
		...
		/**
		 * Notification that a child node was removed from the preference
		 * hierarchy. The given event must not be <code>null</code>.
		 * 
		 * @param event
		 *            an event specifying the details about the removed node
		 * @see IEclipsePreferences.NodeChangeEvent
		 * @see IEclipsePreferences#addNodeChangeListener(IEclipsePreferences.INodeChangeListener)
		 * @see IEclipsePreferences#removeNodeChangeListener(IEclipsePreferences.INodeChangeListener)
		 */
	...
	public final class PreferenceChangeEvent extends EventObject {
		...
		/**
		 * Constructor for a new preference change event. The node and the key
		 * must not be <code>null</code>. The old and new preference values must
		 * be either a <code>String</code> or <code>null</code>.
		 * 
		 * @param node
		 ...
		 *            the preference key
		 * @param oldValue
		 *            the old preference value, as a <code>String</code> or
		 *            <code>null</code>
		 * @param newValue
		 *            the new preference value, as a <code>String</code> or
		 *            <code>null</code>
		 */
		public PreferenceChangeEvent(Object node, String key, Object oldValue,
				Object newValue) {
			super(node);
		 ...
		 * <code>String</code>, or <code>null</code> if the preference was
		 * removed.
		 * 
	...
	public interface IPreferenceChangeListener {
		...
		/**
		 * Notification that a preference value has changed in the preference
		 * store. The given event object describes the change details and must
	...
	/**
	 * Register the given listener for changes to this node. Duplicate calls to
	 * this method with the same listener will have no effect. The given
	 ...
	 * for this node. Calling this method with a listener which is not
	 * registered has no effect. The given listener argument must not be
	 * <code>null</code>.
	 * 
	 * @param listener
	...
	/**
	 * Register the given listener for notification of preference changes to
	 * this node. Calling this method multiple times with the same listener has
	 * no effect. The given listener argument must not be <code>null</code>.
	 * 
	 * @param listener
	 ...
	 * <p>
	 * Functionally equivalent to calling {@link Preferences#removeNode()}. See
	 * the spec of {@link Preferences#removeNode()} for more details.
	 * </p>
	 * <p>
	 * Implementors must send the appropriate {@link NodeChangeEvent} to
	 * listeners who are registered on this node's parent.
	 * </p>
	 * <p>
	 ...
	 *             if there was a problem removing this node
	 ...
	 * </p>
	 * <p>
	 * Note that if the node does not yet exist and is created, then the
	 * appropriate {@link NodeChangeEvent} must be sent to listeners who are
	 ...
	 * method visits this node's children.
	 * 
	 * @param visitor
	 *            the visitor

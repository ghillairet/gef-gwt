...
public final class SafeRunner {
	...
	private static void handleException(ISafeRunnable code, Throwable e) {
		if (!(e instanceof OperationCanceledException)) {
			// try to obtain the correct plug-in id for the bundle providing the
			// safe runnable
			...
			// String message = NLS.bind(CommonMessages.meta_pluginProblems,
			// pluginId);
			// IStatus status;
			// if (e instanceof CoreException) {
			// status = new MultiStatus(pluginId,
			// IRuntimeConstants.PLUGIN_ERROR, message, e);
			// ((MultiStatus) status).merge(((CoreException) e).getStatus());
			// } else {
			// status = new Status(IStatus.ERROR, pluginId,
			// IRuntimeConstants.PLUGIN_ERROR, message, e);
			// }
			// // Make sure user sees the exception: if the log is empty, log
			// the exceptions on stderr

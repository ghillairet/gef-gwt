...
public abstract class AbstractEditPartViewer implements EditPartViewer {
	...
	protected void hookControl() {
		...
		if (contextMenu != null) {
			Menu cm = contextMenu.createContextMenu(getControl());
			if (cm != null) {
				// final PopupPanel popupPanel = new PopupPanel();
				// if (contextMenu instanceof ContextMenuProvider) {
				// MenuBar menuBar = new MenuBar(true);
				// popupPanel.add(menuBar);
				// ((ContextMenuProvider) contextMenu)
				// .buildContextMenu(contextMenu);
				// IContributionItem[] items = contextMenu.getItems();
				// for (final IContributionItem iContributionItem : items) {
				// if (iContributionItem.isSeparator()) {
				// // menuBar.addSeparator();
				// } else if (iContributionItem instanceof
				// ActionContributionItem) {
				// String text = ((ActionContributionItem)
				// iContributionItem)
				// .getAction().getText();
				...
				// }
				// }
				// cm.setPopupPanel(popupPanel);
				// }
				// MenuItem menuItem = new MenuItem(cm, SWT.None);
				// menuItem.setText("dummy item");
				// MenuItem menuItem2 = new MenuItem(cm, SWT.None);
				// menuItem2.setText("dummier item");

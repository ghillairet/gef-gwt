...
public interface IIntroManager {
	 ...
	 *         <code>false</code> otherwise. <code>false</code> is returned if
	 *         part is <code>null</code> or it is not the intro part returned by
	 *         {@link #getIntro()}.
	 */
	 ...
	 * {@link #closeIntro(IIntroPart)} or if there is an intro part but
	 * {@link #showIntro(IWorkbenchWindow, boolean)} has not yet been called to
	 * create it.
	 * 
	...
	/**
	 * Return whether an intro is available. Note that this checks whether there
	 * is an applicable intro part that could be instantiated and shown to the
	 * user. Use {@link #getIntro()} to discover whether an intro part has
	 ...
	 * @param part
	 *            the intro part
	 * @return <code>true</code> if the part in its partially visible standy
	 *         mode, and <code>false</code> if in its fully visible state.
	 *         <code>false</code> is returned if part is <code>null</code> or it
	 *         is not the intro part returned by {@link #getIntro()}.
	 */
	 ...
	 * allow them to work. In full mode, the part should be fully visible and be
	 * the center of the user's attention.
	 * <p>
	 * This method does nothing if the part is <code>null</code> or is not the
	 * intro part returned by {@link #getIntro()}.
	 * </p>
	 * 
	 * @param part
	 *            the intro part, or <code>null</code>
	 * @param standby
	 *            <code>true</code> to put the part in its partially visible
	 ...
	 *            the preferred workbench window, or <code>null</code> to
	 *            indicate the currently active workbench window
	 * @param standby
	 *            <code>true</code> to put the intro part in its partially
	 *            visible standy mode, and <code>false</code> to make it fully
	 *            visible
	 * @return the newly-created or existing intro part, or <code>null</code> if
	 *         no intro part is available or if <code>preferredWindow</code> is
	 *         <code>null</code> and there is no currently active workbench
	 *         window
	 */

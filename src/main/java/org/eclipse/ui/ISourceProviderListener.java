...
public interface ISourceProviderListener {
	 ...
	 *            source values (<code>Object</code>). The names should never be
	 *            <code>null</code>, but the values may be. The map must not be
	 *            <code>null</code>, and should contain at least two elements
	 *            (one for each source).

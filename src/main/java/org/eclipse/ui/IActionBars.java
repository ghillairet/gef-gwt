...
public interface IActionBars {
	 ...
	 * calling <code>updateActionBars</code> so that the changes can be
	 * propagated throughout the workbench.
	 * </p>
	 */
	 ...
	 * manager are responsible for calling <code>updateActionBars</code> so that
	 * the changes can be propagated throughout the workbench.
	 * </p>
	 * 
	 ...
	 * <p>
	 * Note: Clients who add or remove items from the returned tool bar manager
	 ...
	 *            an action id declared in the registry
	 * @param handler
	 *            an action which implements the action id, or <code>null</code>

...
import org.eclipse.ui.internal.services.Expression;
...
public interface IContextService extends IServiceWithSources {
	 ...
	 * example, a service retrieved from a <code>IWorkbenchPartSite</code> would
	 * deactivate all of its contexts when the site is destroyed.
	 * </p>
	 * 
	 * @param contextId
	 ...
	 * {@link #activateContext(String, Expression, boolean)} with global==
	 * <code>false</code>.
	 * </p>
	 * <p>
	 ...
	 * service will be cleaned up when that services is destroyed. So, for
	 * example, a service retrieved from a <code>IWorkbenchPartSite</code> would
	 * deactivate all of its handlers when the site is destroyed.
	 * </p>
	 * 
	 * @param contextId
	 *            The identifier for the context which should be activated; must
	 *            not be <code>null</code>.
	 * @param expression
	 *            This expression must evaluate to <code>true</code> before this
	 *            context will really become active. The expression may be
	 ...
	 * Activates the given context within the context of this service. The
	 * context becomes active when <code>expression</code> evaluates to
	 * <code>true</code>. If global==<code>false</code> then this service must
	 * also be the active service to activate the context.
	 * </p>
	 * <p>
	 * Also, it is guaranteed that the context submitted through a particular
	 * service will be cleaned up when that services is destroyed. So, for
	 ...
	 *         <code>null</code> if no active contexts have been set yet. If the
	 *         set is not <code>null</code>, then it contains only instances of
	 *         <code>String</code>.
	 * @since 3.2
	 */
	 ...
	 * in the workbench.
	 * 
	 * @return The collection of context identifiers (<code>String</code>) that
	 *         are defined; never <code>null</code>, but may be empty.
	 */

			...
			if (div % divsPerMajorMark == 0) {
				...
				if (isHorizontal()) {
					...
					Point textLocation = new Point(
							clippedBounds.x + textMargin, y);
					graphics.pushState();
					int textWidth = FigureUtilities.getTextWidth(num,
							graphics.getFont());
					graphics.translate(textLocation.translate(0, textWidth / 2));
					graphics.rotate(90);
					// SzI:
					graphics.setClip(new Rectangle(0, 0, clippedBounds.height,
							clippedBounds.width));

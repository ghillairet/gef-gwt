...
public interface IScopeContext {
	...
	/**
	 * Return the name of the scope that this context is associated with. Must
	 ...
	 * qualifier or <code>null</code> if the node cannot be determined. The
	 * given qualifier must not be <code>null</code> but may be a path to a
	 * sub-node within the scope.
	 * <p>
	 * An example of a qualifier in Eclipse 2.1 would be the plug-in identifier
	 * that the preference is associated with (e.g. the
	 * "org.eclipse.core.resources" plug-in defines the "description.autobuild"
	 * preference).
	 * </p>
	 * <p>
	 * This method can be used to determine the appropriate preferences node to
	 * aid in setting key/value pairs. For instance:
	 ...
	 * write files that will have the same sharing/scope properties as

...
public interface IPreferencesService {
	...
	/**
	 * Lookup the given key in the specified preference nodes in the given
	 * order. Return the value from the first node the key is found in. If the
	 * key is not defined in any of the given nodes, then return the specified
	 * default value.
	 * <p>
	 * Immediately returns the default value if the node list is
	 * <code>null</code>. If any of the individual entries in the node list are
	 * <code>null</code> then skip over them and move on to the next node in the
	 * list.
	 * </p>
	 * 
	 * @param key
	 ...
	 *            the default value
	 * @param nodes
	 *            the list of nodes to search, or <code>null</code>
	 ...
	 * call the {@link #get(String, String, Preferences[])} method. The order of
	 * the nodes is calculated by consulting the default scope lookup order as
	 * set by {@link #setDefaultLookupOrder(String, String, String[])}.
	 * </p>
	 * <p>
	 ...
	 * lookup order, not by the order of the scope contexts that are being
	 * passed in. The context objects are only consulted to help determine which
	 * nodes to look in, not the order of the nodes.
	 * </p>
	 * <p>
	 * Callers may specify an array of scope context objects to aid in the
	 * determination of the correct nodes. For each entry in the lookup order,
	 * the array of contexts is consulted and if one matching the scope exists,
	 * then it is used to calculate the node. Otherwise a default calculation
	 * algorithm is used.
	 * </p>
	 * <p>
	 * An example of a qualifier for an Eclipse 2.1 preference is the plug-in
	 * identifier. (e.g. "org.eclipse.core.resources" for
	 ...
	 * @param contexts
	 *            optional context objects to help scopes determine which nodes
	...
	/**
	 * Exports all preferences for the given preference node and all its
	 * children to the specified output stream. It is the responsibility of the
	 * client to close the given output stream.
	 * <p>
	 * If the given export list is <code>null</code> then all preferences for
	 * all sub-nodes of the given node are exported to the given stream.
	 * Otherwise the export list is consulted before exporting each preference
	 * value. If there is a string match then the preference is not exported.
	 * The exclusion can also occur at a per-node level. Wild cards are
	 * <em>not</em> accepted in the excludes list as a basic String compare is
	 * done. The basic algorithm is similar to the following:
	 * 
	 * <pre>
	 ...
	 * </p>
	 * <p>
	 * The values stored in the resulting stream are suitable for later being
	 * read by the by {@link #importPreferences(InputStream)} or
	 * {@link #readPreferences(InputStream)} methods.
	 * </p>
	 * 
	 * @param node
	 *            the node to treat as the root of the export
	 * @param output
	 *            the stream to write to
	 * @param excludesList
	 *            a list of path prefixes to exclude from the export, or
	 *            <code>null</code>
	 ...
	 *             if the node or stream is <code>null</code>
	 ...
	 * store. Existing values are over-ridden by those from the stream. The
	 * stream must not be <code>null</code> and is closed upon return from this
	 * method.
	 * <p>
	...
	/**
	 * Take the given preference tree and apply it to the Eclipse global
	 * preference hierarchy. If a node is an export root, then treat the
	 * operation for that node as an overwrite rather than a merge. That is,
	 * remove the node from the global tree before adding any preferences
	 ...
	 * @throws IllegalArgumentException
	 *             if the preferences are <code>null</code>
	 * @throws CoreException
	 ...
	 * the preferences and their values. The given input stream must not be
	 * <code>null</code>. The result of this function is suitable for passing as
	 * an argument to {@link #applyPreferences(IExportedPreferences)}.
	 * <p>
	...
	/**
	 * Return an array with the default lookup order for the preference keyed by
	 * the given qualifier and simple name. Return <code>null</code> if no
	 * default has been set.
	 * <p>
	 * The lookup order returned is based on an exact match to the specified
	 * qualifier and simple name. For instance, if the given key is non-
	 * <code>null</code> and no default lookup order is found, the default
	 * lookup order for the qualifier (and a <code>null</code> key) is
	 * <em>NOT</em> returned. Clients should call
	 * {@link #getLookupOrder(String, String)} if they desire this behavior.
	 * </p>
	 * 
	 * @param qualifier
	 *            the namespace qualifier for the preference
	 * @param key
	 *            the preference name or <code>null</code>
	...
	/**
	 * Return an array with the lookup order for the preference keyed by the
	 * given qualifier and simple name.
	 * <p>
	 * First do an exact match lookup with the given qualifier and simple name.
	 * If a match is found then return it. Otherwise if the key is non-
	 * <code>null</code> then do a lookup based on only the qualifier and return
	 * the set value. Return the default-default order as defined by the
	 ...
	 * @throws IllegalArgumentException
	 *             if the qualifier is <code>null</code>
	 ...
	 * qualifier and simple name. If the given order is <code>null</code> then
	 * the set ordering (if it exists) is removed.
	 * <p>
	 ...
	 *            the preference name or <code>null</code>
	 * @param order
	 *            the lookup order or <code>null</code>
	 * @throws IllegalArgumentException
	 *             <ul>
	 *             <li>if the qualifier is <code>null</code></li>
	 *             <li>if an entry in the order array is <code>null</code> (the
	 *             array itself is allowed to be <code>null</code></li>
	 *             </ul>
	 * @see #getDefaultLookupOrder(String, String)
	 */
	...
	/**
	 * Export the preference tree rooted at the given node, to the specified
	 * output stream. Apply the given list of preference filters, only exporting
	 * preference node and keys which are applicable to at least one filter in
	 * the list.
	 * <p>
	 * The given node and output stream must not be <code>null</code>. If the
	 * list of filters is <code>null</code> or empty then do nothing.
	 * </p>
	 * <p>
	...
	/**
	 * Return a list of filters which match the given tree and is a subset of
	 * the given filter list. If the specified list of filters is
	 * <code>null</code>, empty, or there are no matches, then return an empty
	 * list.
	 * 
	 * @param node
	 *            the tree to match against
	 * @param filters
	 *            the list of filters to match against
	 ...
	 * preference tree. The list of preference filters will act as a filter and
	 * only preferences in the tree which apply to at least one filter in the

...
public class SubActionBars extends EventManager implements IActionBars {
	...
	// * action id ({@link String}) indexed by service locator (
	// * {@link IServiceLocator}). This value is <code>null</code> if there are
	// no
	// * activations.
	// */
	...
	// */
	// protected SubToolBarManager createSubToolBarManager(IToolBarManager
	// parent) {
	...
	//
	// /**
	// * Returns the abstract menu manager. If items are added or removed from
	// the
	...
	// *
	// * @return an <code>IServiceLocater</code> or the parents if the receiver
	// * does not have one
	// *
	// * @since 3.2
	// */
	...
	// public IStatusLineManager getStatusLineManager() {
	// if (statusLineMgr == null) {
	// statusLineMgr = new SubStatusLineManager(
	...
	// actionIdByCommandId.remove(activation
	// .getCommandId());
	// if (service != null) {
	// service.deactivateHandler(activation);
	// }
	...
	// // XXX add new API in next release to avoid down-casting
	// // (bug 137091)
	// if (this instanceof EditorActionBars) {
	// handlerExpression = ((EditorActionBars) this)
	...
	// .getService(IHandlerService.class);
	// if (activationsByActionIdByServiceLocator != null) {
	// final Map activationsByActionId = (Map)
	// activationsByActionIdByServiceLocator
	// .get(serviceLocator);
	// if ((activationsByActionId != null)

...
/**
 * Manages the decorators contributed via the decorators extension point. Views
 * which allow decoration of their elements should use the label decorator
 * returned by <code>getLabelDecorator()</code>.
 * <p>
...
public interface IDecoratorManager extends IDelayedLabelDecorator {
	 ...
	 * enabled decorators. Views which allow decoration of their elements should
	 * use this label decorator. This decorator should be disposed when it is no
	 * longer referenced by the caller or the images created within it may be
	 * kept until {@link JFaceResources#getResources()} is disposed.
	 * 
	...
	/**
	 * Return the IBaseLabelProvider that corresponds to the decoratorId. This
	 * can handle both lightweight and full decorators.
	 * 
	 * @param decoratorId
	 *            the decorator id
	 ...
	 *            the decorator id
	 * @param enabled
	 *            <code>true</code> to enable the decorator, or
	 ...
	 * decoratorID if it exists and is enabled using the IBaseLabelProvider as
	 * the argument to the event. Otherwise do nothing.
	 * <p>

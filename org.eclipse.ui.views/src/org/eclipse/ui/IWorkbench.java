...
public interface IWorkbench extends IAdaptable, IServiceLocator {
	 ...
	 * <code>openPerspective</code> APIs to programmatically show a perspective
	 ...
	 * requested perspective open and active, then the window is given focus.</li>
	 ...
	 * preference for opening perspectives, and that window is given focus.</li>
	 * </ul>
	 * </p>
	 * <p>
	 ...
	 * </p>
	 * <p>
	 * Note that if the element implements <code>IAdaptable</code>, decorators
	 * may use this mechanism to obtain an adapter (for example an
	 ...
	 * Opens a dialog to prompt the user if <code>confirm</code> is true. Return
	 * true if successful. Return false if the user has canceled the command.
	 * 
	 * @since 3.3
	 * 
	 * @param shellProvider
	 ...
	 *            needs to be saved or <code>null</code> if all dirty saveables
	 *            should be saved.
	 * @param confirm
	 *            <code>true</code> to ask the user before saving unsaved
	 *            changes (recommended), and <code>false</code> to save unsaved

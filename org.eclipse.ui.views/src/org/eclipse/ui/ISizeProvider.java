...
public interface ISizeProvider {
	 ...
	 * computePreferredSize should be used. When called with horizontal=true,
	 * this indicates the usage of computePreferredSize(true,...) for computing
	 * widths. When called with horizontal=false, this indicates the usage of
	 * computeSize(false,...) for computing heights. These flags are used for
	 * optimization. Each flag gives the part more control over its preferred
	 * size but slows down the layout algorithm. Parts should return the minimum
	 * set of flags necessary to specify their constraints.
	 * <p>
	 ...
	 * third argument to computePreferredSize will always be set to INFINITE.
	 * The perpendicular size is expensive to compute, and it is usually only
	 * used for wrapping parts.
	 ...
	 * flag should be used sparingly. It can prevent layout caching and cause
	 * the workbench layout algorithm to degrade to exponential worst-case
	 * runtime. If this flag is omitted, then computePreferredSize may be used
	 * to compute the minimum and maximum sizes, but not for anything in
	 * between.</li>
	 * </ul>
	 * 
	 * @param width
	 *            a value of true or false determines whether the return value
	 *            applies when computing widths or heights respectively. That
	 *            is, getSizeFlags(true) will be used when calling
	 *            computePreferredSize(true,...)
	 * @return any bitwise combination of SWT.MAX, SWT.MIN, SWT.WRAP, and
	 *         SWT.FILL
	 */
